...
-- wht is the tot amt each customer spent on zomato?

select a.userid, sum(b.price) total_amt_spent from sales a inner join product b on a.product_id=b.product_id
group by a.userid

-- how many days has each cusomer visited zomato?

select userid, count(distinct created_date) distinct_days from sales group by userid;

-- wht was the first product purchased by each customer?

select * from
(select *, rank() over(partition by userid order by created_date) rnk from sales) a where rnk=1

-- wht is the most purchased item on the menu and how many times was it purchased by all customers?
...
-- which items was purchased just before the customer became a member?
...
-- If buying each product generated points for ex 5rs=2 ZOMATO points and each product has different purchasing points for ex for p1 5rs=1 zomato point, for p2 10rs=5 zomato point and p3 5rs=1 zomato point
...
(select e.*,ant/points total_points from
(select d.*,case when product_id=1 then 5 when product_id=2 then 2 when product_id= 3 then 5 else 0 end as points from
(select c.userid,c.product_id,sum(price) ant from
(select a.*,b.price from sales a inner join product b on a.product_id=b.product_id) c
group by userid,product_id)d)e)f group by product_id)f)g where rnk=1;


-- In the first one year after a customer joins the gold program (including their join date) irrespective of what the customer has purchased they earn 5 zomato points for every 10 rs spent who earned more 1 or 3 and what was their points earnings in their first yr?
--1zp=2rs
--0.5zp=1rs
