...
Calculate "Surgery" Group vs. "Non Surgery" PSM（Based on age, gender, bmi, cancer_stage, smoking_status）。 Nearest neighbor matching was performed to ensure a similar distribution of covariates between the two groups of patients. Calculate the ATE (causal effect of surgery) after matching.
...
-   **p-value = 0.816** → Suggests that Surgery has no statistically significant causal effect on Survival Rate.

-   **R² is close to 0** → Indicates that the model has very little explanatory power, suggesting the presence of unobserved confounders.
...
### **📌 Issue: Highly Concentrated Propensity Scores**
...
### **📌 Why is the Propensity Score Poorly Distributed?**
...
### **📌 Implementing Causal Random Forest: Challenges & Approach**
...
-   **What does ATE tell us?**
    ...
    -   If **ATE ≈ 0**, surgery **has no overall effect**.
